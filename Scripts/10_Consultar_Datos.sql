...
SELECT SalesOrderID, OrderDate, CustomerID, SubTotal, TaxAmt, TotalDue
 ...
 WHERE OrderDate >= '2005-08-01' AND OrderDate <= '2008-08-30'
...
-----------------------------------------------------------------------
4.1 Listar los contactos a los que se les desconoce el número telefónico

SELECT BusinessEntityID, Title, FirstName, LastName
 FROM Person.Person
 WHERE [Title] IS NULL OR LTrim([Title]) = ''
